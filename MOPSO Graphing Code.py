...
# ======================================================
# MOPSO GRAPHING SCRIPT — VERSION 1.0
# Mirrors the structure of NSGA_3_with_graphs.py
# Run ONLY after MOPSO_WITH_new_structure.py has
# already produced the four CSV files below:
#   • mopso_pareto_results.csv
#   • mopso_validation_data.csv
#   • mopso_convergence_data.csv
#   • mopso_sensitivity_data.csv
# ======================================================
...
save_folder = 'publication_plots_mopso'
...
# ── LOAD DATA ──────────────────────────────────────────
try:
    pareto_df = pd.read_csv('mopso_pareto_results.csv')
    valid_df  = pd.read_csv('mopso_validation_data.csv')
    conv_df   = pd.read_csv('mopso_convergence_data.csv')
    sens_df   = pd.read_csv('mopso_sensitivity_data.csv')
    print("All CSV files loaded successfully!\n")
except FileNotFoundError as e:
    print(f"Error: {e}")
    print("Run MOPSO_WITH_new_structure.py first to generate the CSV files.")
    raise SystemExit
...
def plot_parity_grid(df):
    ...
    fig.suptitle('GPR Model Accuracy — Experimental vs. Predicted (MOPSO)', fontsize=16, y=1.01)
    plt.tight_layout()
...
def plot_residuals(df):
    ...
    axes[-1].set_visible(False)
    fig.suptitle('Residual Error Distribution — MOPSO GPR Models', fontsize=16, y=1.01)
    plt.tight_layout()
...
# ══════════════════════════════════════════════════════
# 3. CONVERGENCE + ARCHIVE SIZE (two-panel)
#    Tracks optimization health over iterations.
# ══════════════════════════════════════════════════════
def plot_convergence(df):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

    ax1.plot(df['Iteration'], df['Mean_Desirability'],
             color='darkorange', lw=2.5, marker='o', markersize=3)
    ax1.set_title('MOPSO Convergence — Mean Desirability')
    ax1.set_xlabel('Iteration')
    ax1.set_ylabel('Mean Desirability of Archive')
    ax1.grid(alpha=0.4)

    ax2.plot(df['Iteration'], df['Archive_Size'],
             color='steelblue', lw=2.5, marker='s', markersize=3)
    ax2.set_title('Pareto Archive Growth')
    ax2.set_xlabel('Iteration')
    ax2.set_ylabel('Archive (Pareto) Size')
    ax2.grid(alpha=0.4)

    plt.tight_layout()
    path = f'{save_folder}/convergence_and_archive.png'
    plt.savefig(path, bbox_inches='tight')
    print(f"Saved: {path}")
...
#    Reads pre-computed data; no model needed.
# ══════════════════════════════════════════════════════
def plot_sensitivity(df):
    ...
    plt.title('MOPSO: System Sensitivity Analysis at Optimal Point')
...
# ══════════════════════════════════════════════════════
# 5. RADAR CHART (Best vs Archive Mean)
...
def plot_radar_comparison(df):
    ...
    ax.plot(angles, mean_vals, color='orange', lw=2,  label='Archive Mean',  ls='--')

    ax.set_xticks(angles[:-1])
    ax.set_xticklabels(labels, fontsize=12)
    ax.set_yticklabels([])
    ax.set_title('Performance Fingerprint (Normalized)\nMOPSO Best vs Archive Mean',
                 pad=20, fontsize=14)
...
def plot_pareto_scatter_matrix(df):
    ...
    fig.suptitle('MOPSO Pareto Front — Trade-off Slices', fontsize=16)
    plt.tight_layout()
...
#    Objective + input correlations across Pareto archive.
# ══════════════════════════════════════════════════════
def plot_correlation(df):
    ...
    plt.title('Correlation Matrix — MOPSO Pareto Archive\n(Inputs + Objectives)',
              fontsize=14)
    plt.tight_layout()
...
def plot_parallel_coordinates(df):
    ...
    ax.set_title('Parallel Coordinates — MOPSO Pareto Archive\n(coloured by Desirability tier)',
                 fontsize=14)
...
def plot_3d_pareto(df):
    fig = px.scatter_3d(
        ...
        title='MOPSO — Interactive 3D Pareto Front (BSFC / NOx / BTE)'
    )
...
def print_best_solutions(df, top_n=5):
    ...
    print("\n" + "=" * 70)
    print(f"TOP {top_n} MOPSO SOLUTIONS (ranked by Desirability)")
    print("=" * 70)
